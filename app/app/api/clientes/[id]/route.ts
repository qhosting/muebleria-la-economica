...
export async function PUT(
  ...
  try {
    ...
    const cliente = await prisma.cliente.update({
      where: { id: params.id },
      data: {
        ...(codigoCliente && { codigoCliente: codigoCliente.trim().toUpperCase() }),
        ...(nombreCompleto && { nombreCompleto: nombreCompleto.trim().toUpperCase() }),
        telefono: telefono !== undefined ? (telefono?.trim() || null) : undefined,
        vendedor: vendedor !== undefined ? (vendedor ? vendedor.trim().toUpperCase() : null) : undefined,
        cobradorAsignadoId: cobradorAsignadoId || null,
        statusCuenta,
        ...(direccionCompleta && { direccionCompleta: direccionCompleta.trim().toUpperCase() }),
        ...(descripcionProducto && { descripcionProducto: descripcionProducto.trim().toUpperCase() }),
        diaPago: diaPago,
